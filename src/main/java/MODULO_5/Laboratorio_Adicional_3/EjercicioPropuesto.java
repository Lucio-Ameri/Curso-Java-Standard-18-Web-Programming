...
public class EjercicioPropuesto{
    ...
    public static void main(String[] args){
        ...
        DiccionarioCursos diccionarioCursos = crearDiccionarioCursos();

        try{
            cargarDatosAutomaticos(personas, diccionarioCursos);
        }
        ...
        mostrarDiccionarioCursos(diccionarioCursos);

        System.out.println("\n===== DATOS CARGADOS AUTOMÁTICAMENTE =====");
        mostrarPersonas(personas);

        cargarAlumnos(teclado, personas, diccionarioCursos);
        cargarProfesores(teclado, personas, diccionarioCursos);
    ...
    private static DiccionarioCursos crearDiccionarioCursos(){
        DiccionarioCursos diccionarioCursos = new DiccionarioCursos();

        diccionarioCursos.agregarCurso("Programación I");
        diccionarioCursos.agregarCurso("Programación Orientada a Objetos");
        diccionarioCursos.agregarCurso("HTML");
        diccionarioCursos.agregarCurso("CSS");
        diccionarioCursos.agregarCurso("JavaScript");
        diccionarioCursos.agregarCurso("Base de Datos I");
        diccionarioCursos.agregarCurso("SQL");

        return diccionarioCursos;
    }

    private static void cargarDatosAutomaticos(Set<Persona> personas, DiccionarioCursos diccionarioCursos) throws DocumentoInvalidoException {
        List<Integer> idsCursosProgramacion = obtenerIdsCursos(diccionarioCursos, "Programación I", "Programación Orientada a Objetos");
        List<Integer> idsCursosWeb = obtenerIdsCursos(diccionarioCursos, "HTML", "CSS", "JavaScript");
        List<Integer> idsCursosBaseDatos = obtenerIdsCursos(diccionarioCursos, "Base de Datos I", "SQL");
        ...
        Profesor profesor1 = new Profesor("Andrés", "Molina", new Documento("DNI", "30000001"), LocalDate.of(1982, 4, 18), LocalDate.of(2016, 3, 1), 650000.0, idsCursosProgramacion);
        Profesor profesor2 = new Profesor("Patricia", "Suárez", new Documento("DNI", "30000002"), LocalDate.of(1979, 12, 9), LocalDate.of(2017, 4, 1), 670000.0, idsCursosWeb);
        Profesor profesor3 = new Profesor("Ricardo", "Benítez", new Documento("DNI", "30000003"), LocalDate.of(1985, 7, 22), LocalDate.of(2019, 3, 15), 640000.0, idsCursosBaseDatos);

        Alumno alumno1 = new Alumno("Lucía", "Martínez", new Documento("DNI", "40000001"), LocalDate.of(2003, 2, 17), LocalDate.of(2023, 3, 1), idsCursosProgramacion);
        Alumno alumno2 = new Alumno("Mateo", "Herrera", new Documento("DNI", "40000002"), LocalDate.of(2002, 10, 11), LocalDate.of(2022, 3, 1), idsCursosWeb);
        Alumno alumno3 = new Alumno("Camila", "Arias", new Documento("DNI", "40000003"), LocalDate.of(2004, 5, 6), LocalDate.of(2024, 3, 1), idsCursosBaseDatos);
    ...
    private static List<Integer> obtenerIdsCursos(DiccionarioCursos diccionarioCursos, String... nombresCursos){
        List<Integer> idsCursos = new ArrayList<>();

        for(String nombreCurso : nombresCursos){
            Curso curso = diccionarioCursos.buscarPorNombre(nombreCurso);
    ...
    private static void mostrarDiccionarioCursos(DiccionarioCursos diccionarioCursos){
        for(Curso curso : diccionarioCursos.getCursos()){
    ...
    private static void cargarAlumnos(Scanner teclado, Set<Persona> personas, DiccionarioCursos diccionarioCursos){
        String continuar;
        ...
        do{
            ...
            List<Integer> idsCursos = pedirCursos(teclado, diccionarioCursos, "¿Cuántos cursos cursa el alumno?: ");
    ...
    private static void cargarProfesores(Scanner teclado, Set<Persona> personas, DiccionarioCursos diccionarioCursos){
        String continuar;
        ...
        do{
            ...
            List<Integer> idsCursos = pedirCursos(teclado, diccionarioCursos, "¿Cuántos cursos dicta el profesor?: ");
    ...
    private static List<Integer> pedirCursos(Scanner teclado, DiccionarioCursos diccionarioCursos, String mensajeCantidad){
        List<Integer> idsCursos = new ArrayList<>();
        ...
        for(int i = 0; i < cantidadCursos; i++){
            Curso curso = pedirCursoExistente(teclado, diccionarioCursos, "Ingrese nombre del curso " + (i + 1) + ": ");
            idsCursos.add(curso.getId());
        }
    ...
    private static Curso pedirCursoExistente(Scanner teclado, DiccionarioCursos diccionarioCursos, String mensaje){
        Curso curso = null;

        while(curso == null){
            String nombreCurso = pedirTexto(teclado, mensaje);
            curso = diccionarioCursos.buscarPorNombre(nombreCurso);

            if(curso == null){
                System.out.println("Error: el curso ingresado no existe en el diccionario.");
