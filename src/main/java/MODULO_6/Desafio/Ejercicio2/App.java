...
public class App{
    ...
    private static void probarLogDeErroresEnBD(){
        System.out.println("\nPRUEBA DE LOGS EN BASE DE DATOS");
        System.out.println("--------------------------------");

        intentarCrearProductoParaProbarLog("PF");
        intentarCrearProductoParaProbarLog("XYZ");
    }

    private static void intentarCrearProductoParaProbarLog(String codigoProducto){
        try{
            Fabrica.crearProducto(codigoProducto, 999, 999);
        }
        catch(ProductoException e){
            System.out.println("Error capturado: " + e.getMessage() + " | Código: " + e.getCodigoError() + " | Clase: " + e.getClaseAfectada() + " | Producto: " + e.getProductoAfectado());
        }
    }
}