...
public class LoginBDServlet extends HttpServlet{
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
        ...
        String idSesionActual = (sesionActual != null) ? sesionActual.getId() : null;
        if(GestorSesiones.tieneSesionEnOtroNavegador(usuario, idSesionActual)){
            mostrarFormulario(response, "Ya tienes una sesión activa en otro navegador. " + "Debes cerrar esa sesión antes de iniciar sesión aquí.");
            return;
        }

        HttpSession sesion = request.getSession(true);
        sesion.setAttribute("usuario", usuario);
        GestorSesiones.registrarSesion(usuario, sesion.getId());
